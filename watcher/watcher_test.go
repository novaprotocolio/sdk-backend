...
func (s *watcherTestSuit) TestInitLastBlockNumberWithCache() {
	...
	watcher.KVClient.(*common.MockKVStore).On("Get", common.NOVA_WATCHER_BLOCK_NUMBER_CACHE_KEY).Return("10086", nil)
...
func (s *watcherTestSuit) TestInitLastBlockNumberWithoutCache() {
	...
	watcher.KVClient.(*common.MockKVStore).On("Get", common.NOVA_WATCHER_BLOCK_NUMBER_CACHE_KEY).Return("", common.KVStoreEmpty)
	watcher.Nova.(*sdk.MockNova).BlockChain.(*sdk.MockBlockchain).On("GetBlockNumber").Return(uint64(10086), nil)
